            ...
            <main className="container mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-24 space-y-24 md:space-y-32">
                {servicesList.map((service, index) => (
                    <section key={index} id={service.id} ref={addToRefs} className="grid grid-cols-1 lg:grid-cols-2 gap-12 md:gap-20 items-center scroll-reveal">
                        <div className={`relative h-96 lg:h-[500px] w-full overflow-visible ${index % 2 === 1 ? 'lg:order-2' : ''}`}>
                            <div className="absolute inset-0 rounded-4xl shadow-2xl overflow-hidden">
                                <ImageWithLoader 
                                    src={service.image} 
                            ...
                            <div className={`absolute -bottom-8 w-48 bg-white/25 backdrop-blur-lg rounded-3xl hidden lg:flex items-center justify-center py-3 px-5 border border-white/30 shadow-xl ${index % 2 === 1 ? 'left-6' : 'right-6'}`}>
                                <ImageWithLoader
